...
export default function ChatPanel({ onClose }) {
  ...
  // SYSTEM PROMPT: Forces AuraX to follow your specific behavior rules
  const AURAX_INSTRUCTIONS = `
    You are AuraX — an unbiased AI Legal Decision Support System.
    Context: The document has already been uploaded. Treat this content as your primary context.
    NEVER ask for the document again.
    
    Behavior Rules:
    1. Directly extract and answer user questions from the document.
    2. If answer exists: Explain in simple terms, reference clauses, and stay neutral.
    3. If answer is missing: Say "This information is not clearly specified in the document" and suggest what to check.
    4. NO legal advice. Use soft language: "You may consider...", "It might be helpful to review..."
    5. Stay neutral. Highlight what it means, who it affects, and implications.
    6. Style: Clear, structured, simple, and avoid legal jargon.
  `;
  ...
  const send = async (text) => {
    ...
    try {
      // We prepend the instructions to the message so the AI follows the persona
      const messageWithPersona = `${AURAX_INSTRUCTIONS}\n\nUser Question: ${msg}`;

      const response = await sendChatMessage(
        messageWithPersona,
        chatHistory.slice(-8), // Send last 8 messages for context
        rawText.substring(0, 10000), // Increased to 10k characters for better document coverage
        language
      );
